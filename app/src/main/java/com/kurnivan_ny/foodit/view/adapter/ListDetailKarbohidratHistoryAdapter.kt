...
class ListDetailKarbohidratHistoryAdapter(var detailhistoryList: ArrayList<ListDetailHistoryModel>)
    :RecyclerView.Adapter<ListDetailKarbohidratHistoryAdapter.DetailListHistoryViewModel>(){
    ...
    inner class DetailListHistoryViewModel(private val itemBinding: ListItemHistoryBinding)
        :RecyclerView.ViewHolder(itemBinding.root){
            fun bind(data: ListDetailHistoryModel){
                ...
                itemBinding.tvDescription.text = "${String.format("%.2f", (data.karbohidrat.toString() + "F").toFloat())} gr"
            }
        }
